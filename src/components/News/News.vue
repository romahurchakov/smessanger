<template>
  <div class="news_page">
    <div class="news_page__header">
      <div class="news_page__header__search">
        <el-autocomplete
          v-model="search"
          ...
          @select="handleSelect"
          class="news_page__header__input"
        >
      ...
      <div class="news_page__header__create">
        <Button type="simple" label="Создать новость" @click="createNews" />
      </div>
    </div>
    <el-dialog title="Хотите удалить группу" :visible.sync="showDeletePopup" width="550px">
      ...
      <div slot="footer" class="btn-footer">
        ...
        />
        <Button type="simple" label="Да, удалить" width="150" @click="deleteNews" />
      </div>
    </el-dialog>
    <div class="news_page__body">
      <div class="news_page__card" v-for="(elem, index) in news" :key="elem.id">
        <div class="news_page__card__header">
          <p
            style="margin-right: auto; font-size:20px; font-family: Arial, Helvetica, sans-serif;"
          >{{ elem.title }}</p>
          <i
            class="el-icon-close delete-hover"
            @click="showDeletePopup = true; to_delete = news[index].id"
          ></i>
        </div>
        <div class="news_page__card__internal">
          ...
          <div
            v-if="elem.doc_name"
            class="news_page__card__internal__doc"
